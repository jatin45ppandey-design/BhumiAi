...
class DuplicateResolutionTests(unittest.TestCase):
    def setUp(self):
        ...
        with self.sessions() as db:
            ...
            owner = models.User(name="Original Owner", email="owner@example.test", role="user")
            uploader = models.User(name="New Uploader", email="new@example.test", role="user")
            unrelated = models.User(name="Unrelated", email="unrelated@example.test", role="user")
    ...
    def test_unique_file_reports_none_without_processing(self):
        with patch.object(officer, "recognize_fast_khatauni", side_effect=AssertionError("duplicate check must not OCR")), \
            ...
            response = self.client.post(f"/api/documents/{self.unique_doc_id}/duplicate-check", params={"officer_id": self.officer_id})
    ...
    def test_exact_match_returns_verified_reference_and_never_auto_rejects(self):
        response = self.client.post(f"/api/documents/{self.current_doc_id}/duplicate-check", params={"officer_id": self.officer_id})
        self.assertEqual(response.status_code, 200)
        ...
        self.assertTrue(self.client.get("/api/user/notifications", params={"user_id": self.owner_id}).json() == [])
    ...
    def test_rejected_match_includes_its_existing_rejection_reason(self):
        ...
        response = self.client.post(f"/api/documents/{current_doc_id}/duplicate-check", params={"officer_id": self.officer_id})
        self.assertEqual(response.status_code, 200)
        payload = response.json()
    ...
    def test_continue_override_preserves_both_submissions(self):
        self.client.post(f"/api/documents/{self.current_doc_id}/duplicate-check", params={"officer_id": self.officer_id})
        response = self.client.post(f"/api/documents/{self.current_doc_id}/duplicate-continue", params={"officer_id": self.officer_id, "matched_document_id": self.existing_doc_id})
        self.assertEqual(response.status_code, 200)
        with self.sessions() as db:
    ...
    def test_duplicate_rejection_notifies_owner_without_exposing_uploader(self):
        self.client.post(f"/api/documents/{self.current_doc_id}/duplicate-check", params={"officer_id": self.officer_id})
        rejected = self.client.post(f"/api/officer/documents/{self.current_doc_id}/reject", params={"officer_id": self.officer_id}, json={"reason_category": "Duplicate Submission", "officer_note": "This submission exactly matches existing verified record LR-UP-LUC-000014."})
        self.assertEqual(rejected.status_code, 200)
        uploader_rows = self.client.get("/api/user/submissions", params={"user_id": self.uploader_id, "status": "REJECTED"}).json()
        ...
        notices = self.client.get("/api/user/notifications", params={"user_id": self.owner_id}).json()
        self.assertEqual(len(notices), 1)
        self.assertIn("LR-UP-LUC-000014", notices[0]["message"])
        self.assertNotIn("New Uploader", notices[0]["message"])
        self.assertEqual(self.client.get("/api/user/notifications", params={"user_id": self.unrelated_id}).json(), [])
