...
export const api = {
  ...
  userSubmissions: (id, {status = '', search = ''} = {}) => request(`/api/user/submissions?${new URLSearchParams({user_id: id, ...(status ? {status} : {}), ...(search ? {search} : {})})}`),
  ...
  decision: (id, action, officerId, payload) => request(`/api/officer/documents/${id}/${action}?officer_id=${officerId}`, payload ? jsonRequest('POST', payload) : {method:'POST'}),
  duplicateCheck: (id, officerId) => request(`/api/documents/${id}/duplicate-check?officer_id=${encodeURIComponent(officerId)}`, {method:'POST'}),
  duplicateContinue: (id, officerId, matchedDocumentId) => request(`/api/documents/${id}/duplicate-continue?officer_id=${encodeURIComponent(officerId)}&matched_document_id=${encodeURIComponent(matchedDocumentId)}`, {method:'POST'}),
  notifications: (userId) => request(`/api/user/notifications?user_id=${encodeURIComponent(userId)}`),
  markNotificationRead: (userId, notificationId) => request(`/api/user/notifications/${notificationId}/read?user_id=${encodeURIComponent(userId)}`, {method:'PATCH'}),
