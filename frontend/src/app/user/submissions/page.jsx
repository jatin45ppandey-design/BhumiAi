...
export default function Submissions() {
  ...
  return <>
    ...
    <div className="search"><input aria-label="Search my submissions" value={q} onChange={event => setQ(event.target.value)} onKeyDown={event => event.key === 'Enter' && load()} placeholder="Search document, record ID, village, or status…"/>{q && <button type="button" className="search-clear" onClick={() => { setQ(''); load(status, ''); }}><X size={15}/> Clear</button>}<Button onClick={() => load()} loading={searching} loadingText="Searching…"><Search size={16}/> Search</Button></div>
