...
function DigitalKhatauni({ digitization, digitizationLoading, work, isTerminal, typingMode, setTypingMode, drafts, isDirty, error, onFieldChange, onCellChange, onAddRow, onDeleteRow, onSave, onNeedsReview, onVerify, onReject }) {
  ...
  return <section className="card khatauni-pane">
    ...
    <div className="khatauni-fields" onChangeCapture={(event) => {
      ...
      {items.length ? items.map((field) => <label className="khatauni-field" data-field-id={field.id} key={field.id}>
...
export default function Review() {
  ...
  return <>
    ...
    {isDirty && <div className="unsaved-changes" role="status">Unsaved changes</div>}
    {['approve', 'mark-review', 'reject'].includes(work) && <div className="review-action-progress" role="status">{work === 'approve' ? 'Verifying...' : work === 'reject' ? 'Rejecting...' : 'Updating...'}</div>}
